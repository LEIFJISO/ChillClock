...
- 設定に「集中中は終了/スキップ禁止」「集中中は右側UIを隠す」「集中中はゲーム終了禁止」を追加(後者は Steam の終了も止めます。Steam が終了するとゲームごと強制終了されるため。Steam をホワイトリストに入れるとこの制限は解除されます);
- 集中を妨げる操作やタスクマネージャーの起動、ゲーム終了を試みた時にサトネがボイスで注意;
- 設定UIは簡体中文 / English / 日本語に対応。

**サトネのボイス(全 1722 条、すべて DLL に内蔵)**
...
| ゲーム終了時 | 110 | 30 | 80 |
| ミニ授業のお話(通常時 / 休憩の雑談、クリック) | 289 | 289 | 0 |
| **合計** | **1722** | **749** | **973** |

> 「連続」は何文かを続けて話す連続ボイスで、全 260 組。うち 53 組が「ミニ授業」のお話(1 組 4〜20 文)です。「ミニ授業」の 289 条は、通常時のクリック / 休憩中のクリック / 通常時の雑談 / 休憩の雑談の各プールに入ります。ほかに 36 条の季節イベント用ボイスがあり、当日に一度だけ、朝 / 昼 / 夕 / 夜で区別して再生されます。ボイスパックは `ChillClock.dll` に内蔵されているので、別途ボイスフォルダは不要です。
